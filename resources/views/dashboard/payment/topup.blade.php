...
@section('content')
    <div class="card">
        <div class="card-body">
            <table id="topupTable" class="table table-bordered table-hover">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>User</th>
                ...
                <tbody>
                    @foreach ($transactions as $trx)
                        <tr>
                            ...
                            <td>
                                <span class="text-{{ $trx->operation == 'plus' ? 'success' : 'danger' }}">
                                    {{ $trx->operation == 'plus' ? '+' : '-' }}Rp {{ number_format($trx->amount, 0, ',', '.') }}
                                </span>
                            </td>
                            <td>
                                @if ($trx->proof_url)
                                    <a href="{{ asset($trx->proof_url) }}" target="_blank">
                                        <img src="{{ asset($trx->proof_url) }}" width="70" class="img-thumbnail">
                                    </a>
                                @else
                                    <em>Tidak Ada</em>
                                @endif
